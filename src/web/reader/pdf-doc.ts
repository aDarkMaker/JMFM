...
async function loadNativeRange(pdfjs: PdfjsModule, filePath: string): Promise<DocCacheEntry> {
  ...
  const range = new (class extends pdfjs.PDFDataRangeTransport {
...
export async function loadDoc(filePath: string): Promise<DocCacheEntry> {
  ...
  if (Capacitor.isNativePlatform()) {
    try {
      const entry = await loadNativeRange(pdfjs, filePath);
